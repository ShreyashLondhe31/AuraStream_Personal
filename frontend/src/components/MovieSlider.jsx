...
const MovieSlider = ({category}) => {
    ...
    const formattedCategoryType = category.replaceAll("_", " ")[0].toUpperCase() + category.replaceAll("_", " ").slice(1)
    ...
    <>
        <div className='text-white bg-black relative px-5 md:px-20' onMouseEnter={() => setShowArrows(true)} onMouseLeave={() => setShowArrows(false)}>
            ...
            <div className='flex space-x-4 overflow-x-scroll no-scrollbar' ref={sliderRef} >
                {content.map((item) => (
                    <Link to={`/watch/${item.id}`} className='min-w-[250px] realtive group' key={item.id}> 
